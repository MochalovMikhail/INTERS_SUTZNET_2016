...
namespace SUTZ_2.MobileSUTZ
{
    ...
    class MobileSUTZ_Rashod
    {
        ...
        private IQueryable<MobileWorkUnit> findUnitWithOrWithoutCurrentUser(IQueryable<MobileWorkUnit> listOfUnitsAll, byte flFilterUser)
        {
            ...
            if (scanStateFields.idDocSpecRashCurrent > 0)
            {
                returnValue = from o in listOfUnitsAll where o.BaseDoc.idd == scanStateFields.idDocSpecRashCurrent select o;
            }
        ...
        /// <summary>
        /// Метод вызывается при отмене задания.
        /// </summary>
        /// <returns></returns>
        private bool onCancelUnitForWork(MobileWorkUnit mwUnit)
        {
            // установка текущего пользователя, если он не выбран
            mwUnit.AssignedUser = null;            
            mwUnit.TypeOfMobileState = enTypesOfMobileStates.ДоступенДляРаботы;
            if (saveUnitForCurrentUser(mwUnit))
            {
                scanStateFields.refreshAllField();
            }
        ...
        {
            byte rezimRaboty = 0;
            if (mwUnit.JobType.TypeOfWork == enTypeOfWorks.ОтборТовара)
            {
                if (scanStateFields.currentScanState == enumScanState.НачалоРаботы)
                {
                    rezimRaboty = 1;
            ...
            else if (mwUnit.JobType.TypeOfWork == enTypeOfWorks.ОтборВТочкуПередачи)
            {
                if (scanStateFields.currentScanState == enumScanState.НачалоРаботы)
                {
                    rezimRaboty = 1;
            ...
            else if (mwUnit.JobType.TypeOfWork == enTypeOfWorks.ОтборИзТочкиПередачи)
            {
                if (scanStateFields.currentScanState == enumScanState.НачалоРаботы)
                {
                    rezimRaboty = 4;
        ...
        private int processBarcode(String scanedBarcode)
        {
            ...
            if (scanStateFields.currentScanState == enumScanState.ОтсканированПаллетОтправитель)
            {
                ...
                if (tovarsBG.Count() == 0)
                {
                    scanStateFields.strLabelCaptionOne = captionOne + "<br> <size=36> <backcolor=255, 255, 255> <color=" + Color.RoyalBlue.ToArgb() + ">Штрих-кода нет в базе!</color></backcolor></size>";
        ...
        private int onEnterQuantity(ref structScanStringParams paramScan)
        {
            ...
            //logger.Trace("onEnterQuantity: Метод вызван с параметром paramScan=" + paramScan.ToString());
        ...
        private DialogResult lokScanKodRazmesheniya(MobileWorkUnit mwUnit)
        {
            ...
            XPQuery<DocSpecRashodaGoods> xpQuery = new XPQuery<DocSpecRashodaGoods>(objectSpace.Session());
            ...
            else if ((rezimRaboty == 2) || (rezimRaboty == 4))// ПоложитьВ в кладовую отгрузки или при размещении из точки передачи (4)
            {
            ...
            else if (rezimRaboty == 3)// ТочкаПередачи - взять из точки передачи
            {
        ...
        private DialogResult lokEnterMainMoveDialog()
        {
            ...
            XPQuery<DocSpecRashodaGoods> xpQuery = new XPQuery<DocSpecRashodaGoods>(objectSpace.Session());
            var summByPallet = from s in xpQuery
        ...
        private bool runMainLoop()
        {
            ...
            while (true)
            {
                ...
                {
                    #region 1. Получение задания на расход и фиксация его за текущим сотрудником
                    ...
                    scanStateFields.idDocSpecRashCurrent = unitForWork.BaseDoc.idd;
                    ...
                    {
                        bool bUnSaveUnit = onCancelUnitForWork(unitForWork);
                        break;
                    }
                ...
                //{
                //    // ячейку-получатель нужно сканировать, только если есть точка передачи или код в кладовой отгрузки.
                //    // 2. запрос сканирования паллета отправителя: 
                //    DialogResult dlgResult = lokScanKodRazmesheniya(unitForWork);
                //    if (dlgResult == DialogResult.Cancel)
                //    {
        ...
        private bool lokSaveAllDateToDataBase()
        {
            ...
            String strOperName = "lokSaveAllDateToDataBase: Операция: " + scanStateFields.sprJobeType.ToString();

            logger.Trace("Вход в метод: {0} по заданию: {1}", strOperName, scanStateFields.mobileWorkUnitIdd);

            if (scanStateFields.docRowIds == null)
            {
                logger.Error("{0} по документу idd={1}. Ошибка: нет строк документа для сохранения изменений.queryDocRows=null", strOperName, scanStateFields.idDocSpecRashCurrent);
                showUserMessage("Ошибка: нет строк спец.перемещения для сохранения изменений");
                return false;
            }

            //остановился здесь! исправить логи для приведения к единому стандарту - имя метода и тип операции в одной стандарту.
            ...
            if (currentUser == null)
            {
                logger.Error("{0} документ idd={1}. Ошибка определения текущего пользователя. currentUser=null", strOperName, scanStateFields.idDocSpecRashCurrent);
                showUserMessage("Ошибка определения текущего пользователя!");
                return false;
            }
            if (delimeter == null)
            {
                logger.Error("{0} документ idd={1}. Ошибка определения текущего разделителя учета. delimeter=null", strOperName, scanStateFields.idDocSpecRashCurrent);
                showUserMessage("Ошибка определения текущего разделителя учета!");
                return false;
            }
            if (elementJobType == null)
            {
                logger.Error("{0} документ idd={1}. Ошибка определения текущего вида работы. elementJobType=null", strOperName, scanStateFields.idDocSpecRashCurrent);
                showUserMessage("Ошибка определения текущего вида работы!");
                return false;
            }
            if (scanStateFields.idDocSpecRashCurrent == 0)
            {
                logger.Error("{0} документ idd={1}. Ошибка поиска по идд документа задания. idDocSpecRashCurrent=0", strOperName, scanStateFields.idDocSpecRashCurrent);
            ...
            {
                logger.Error("{0} документ idd={1}. idd единицы работы = 0. Невозможно выполнить сохранение результатов работы!", strOperName, scanStateFields.idDocSpecRashCurrent);
            ...
            {
                ObjectType = typeof(DocSpecRashoda).FullName,
                ObjectRow_Id = scanStateFields.idDocSpecRashCurrent,
                Outload = 0,
                User = currentUser,
            ...
            // 3.6.4 установка у строки документа спец.расхода нового статуса проведения:
            XPQuery<DocSpecRashodaGoods> querySPDocTableGoods = new XPQuery<DocSpecRashodaGoods>(objectSpace.Session());
            var listOfDocRows = from c in querySPDocTableGoods where scanStateFields.docRowIds.Contains(c.idd) select c;
            ...
            foreach (DocSpecRashodaGoods item in listOfDocRows)
            {
            ...
            XPQuery<DocSpecRashodaGoods> querySPDocGoodsNotComplete = new XPQuery<DocSpecRashodaGoods>(objectSpace.Session());
            var listOfNotCompletedDocRows = from c in querySPDocTableGoods
                                            where
                                            c.ParentID.idd == scanStateFields.idDocSpecRashCurrent
                                            && c.MoveStatus != enDocRowMoveStatus.ЧП_ПоложитьВ
                                            select c;

            if ((listOfNotCompletedDocRows == null) || (listOfNotCompletedDocRows.Count() == 0))
            {
                DocSpecRashoda currentDocMove = objectSpace.FindObject<DocSpecRashoda>(new BinaryOperator("idd", scanStateFields.idDocSpecRashCurrent, BinaryOperatorType.Equal), true);
                // установка в документе признака "товар отобран"
                currentDocMove.TovarOtobran = true;
                currentDocMove.Save();
                ...
                DocJobesProperties djProp = (from c in djp
                                             where c.BaseDoc.idd == scanStateFields.idDocSpecRashCurrent
                ...
                if (djProp != null)
                {
                    ...
                    djProp.Save();
                    // 3.8.3 выгрузка в 7.7 задания на работу.
        ...
        private bool lokRunCommitChanges()
        {
            ...
            {
                strError = "Ошибка Exception. документ idd=" + scanStateFields.idDocSpecRashCurrent;
        ...
        private class localClassScanState
        {
            ...
            //internal int idDocSpecRashodaFilter { get; set; }
            ...
            internal int idDocSpecRashCurrent { get; set; }
            ...
            // 10. элемент справочника ЗаданияСклада, по которому выполняется работа:
            internal int mobileWorkUnitIdd { get; set; }

            // 11. список идентификаторов строк документа, по которым выполняется работа:
