//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
using System;
using DevExpress.Xpo;
using DevExpress.Data.Filtering;
namespace SUTZ_2.Module.BO.Exchange.SUTZ1C_SUTZNET {

	public partial class SUTZ_NET_DocInventoryGoods : XPLiteObject {
		int frow_id;
		[Key(true)]
		public int row_id {
			get { return frow_id; }
			set { SetPropertyValue<int>("row_id", ref frow_id, value); }
		}
		SUTZ_NET_DocInventoryHeaders fHeaderID;
		[Indexed(Name=@"IX_SUTZ_NET_HeaderID", Unique=true)]
		[Association(@"SUTZ_NET_DocInventoryGoodsReferencesSUTZ_NET_DocInventoryHeaders")]
		public SUTZ_NET_DocInventoryHeaders HeaderID {
			get { return fHeaderID; }
			set { SetPropertyValue<SUTZ_NET_DocInventoryHeaders>("HeaderID", ref fHeaderID, value); }
		}
		int fDocLineNo;
		public int DocLineNo {
			get { return fDocLineNo; }
			set { SetPropertyValue<int>("DocLineNo", ref fDocLineNo, value); }
		}
		Guid fGoodID;
		public Guid GoodID {
			get { return fGoodID; }
			set { SetPropertyValue<Guid>("GoodID", ref fGoodID, value); }
		}
		Guid fGoodIDFact;
		public Guid GoodIDFact {
			get { return fGoodIDFact; }
			set { SetPropertyValue<Guid>("GoodIDFact", ref fGoodIDFact, value); }
		}
		Guid fUnitID;
		public Guid UnitID {
			get { return fUnitID; }
			set { SetPropertyValue<Guid>("UnitID", ref fUnitID, value); }
		}
		Guid fUnitIDFact;
		public Guid UnitIDFact {
			get { return fUnitIDFact; }
			set { SetPropertyValue<Guid>("UnitIDFact", ref fUnitIDFact, value); }
		}
		decimal fTotalQuantity;
		public decimal TotalQuantity {
			get { return fTotalQuantity; }
			set { SetPropertyValue<decimal>("TotalQuantity", ref fTotalQuantity, value); }
		}
		decimal fTotalQuantityFact;
		public decimal TotalQuantityFact {
			get { return fTotalQuantityFact; }
			set { SetPropertyValue<decimal>("TotalQuantityFact", ref fTotalQuantityFact, value); }
		}
		Guid fTakeFrom;
		public Guid TakeFrom {
			get { return fTakeFrom; }
			set { SetPropertyValue<Guid>("TakeFrom", ref fTakeFrom, value); }
		}
		DateTime fBestBefore;
		public DateTime BestBefore {
			get { return fBestBefore; }
			set { SetPropertyValue<DateTime>("BestBefore", ref fBestBefore, value); }
		}
		DateTime fBestBeforeFact;
		public DateTime BestBeforeFact {
			get { return fBestBeforeFact; }
			set { SetPropertyValue<DateTime>("BestBeforeFact", ref fBestBeforeFact, value); }
		}
		decimal fQuantityOfUnits;
		public decimal QuantityOfUnits {
			get { return fQuantityOfUnits; }
			set { SetPropertyValue<decimal>("QuantityOfUnits", ref fQuantityOfUnits, value); }
		}
		decimal fQuantityOfUnitsFact;
		public decimal QuantityOfUnitsFact {
			get { return fQuantityOfUnitsFact; }
			set { SetPropertyValue<decimal>("QuantityOfUnitsFact", ref fQuantityOfUnitsFact, value); }
		}
		decimal fQuantityOfItems;
		public decimal QuantityOfItems {
			get { return fQuantityOfItems; }
			set { SetPropertyValue<decimal>("QuantityOfItems", ref fQuantityOfItems, value); }
		}
		decimal fQuantityOfItemsFact;
		public decimal QuantityOfItemsFact {
			get { return fQuantityOfItemsFact; }
			set { SetPropertyValue<decimal>("QuantityOfItemsFact", ref fQuantityOfItemsFact, value); }
		}
		bool fCheckIn;
		public bool CheckIn {
			get { return fCheckIn; }
			set { SetPropertyValue<bool>("CheckIn", ref fCheckIn, value); }
		}
		Guid fPropertyID;
		public Guid PropertyID {
			get { return fPropertyID; }
			set { SetPropertyValue<Guid>("PropertyID", ref fPropertyID, value); }
		}
		Guid fPropertyIDFact;
		public Guid PropertyIDFact {
			get { return fPropertyIDFact; }
			set { SetPropertyValue<Guid>("PropertyIDFact", ref fPropertyIDFact, value); }
		}
		Guid fDocLineUID;
		public Guid DocLineUID {
			get { return fDocLineUID; }
			set { SetPropertyValue<Guid>("DocLineUID", ref fDocLineUID, value); }
		}
	}

}
